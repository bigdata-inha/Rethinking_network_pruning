...
class ImagenetDataLoader:
    def __init__(self, config):
        ...
        if config.data_mode == "image_folder":
            ...
            train_set = datasets.ImageFolder(os.path.join(self.config.data_dir, "ILSVRC2012_img_256_train"), transform=train_transform)
            valid_set = datasets.ImageFolder(os.path.join(self.config.data_dir, "ILSVRC2012_img_256_val"), transform=valid_transform)
        else:
...
class SpecializedImagenetDataLoader:
    def __init__(self, config, *subset_labels):
        ...
        if config.data_mode == "image_folder":
            ...
            part_and_sample_train_set = datasets.ImageFolder(os.path.join(self.config.data_dir, "ILSVRC2012_img_256_train"), transform=train_transform)
            nc_valid_set = copy.deepcopy(valid_set)
            ...
            for i, e in enumerate(valid_set.targets):
                if last_labels != e:
                    ...
                    if len(oth_idx) > 50 * len(subset_labels):
                        binary_valid_set = torch.utils.data.dataset.Subset(valid_set, subset_idx + oth_idx)
                        binary_nc_valid_set = torch.utils.data.dataset.Subset(nc_valid_set, subset_idx + oth_idx)
                        self.binary_valid_loader.append(DataLoader(binary_valid_set, batch_size=self.config.batch_size))
                        self.binary_nc_valid_loader.append(DataLoader(binary_valid_set, batch_size=self.config.batch_size))
                        oth_idx = []
            ...
            self.binary_valid_iterations = [len(loader) for loader in self.binary_valid_loader]
            self.binary_nc_valid_iterations = self.binary_valid_iterations
        else:
            raise Exception("Please specify in the json a specified mode in data_mode")
